...
def add_progress(subject, topic, topic_number, conspect_path):
    ...
    c.execute(
        "INSERT OR REPLACE INTO progress (subject, topic, topic_number, status, conspect_path, created_at, completed_at) VALUES (?, ?, ?, ?, ?, ?, ?)",
...
def add_term(term, definition, subject, file_path):
    ...
    now = datetime.now().isoformat()
    c.execute(
        "INSERT OR IGNORE INTO terms (term, definition, subject, file_path, created_at) VALUES (?, ?, ?, ?, ?)",
