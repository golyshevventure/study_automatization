...
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "summary_programs"))

from backend.summary.netology_scraper import NetologyScraper
from netology_api_client import NetologyAPIClient
from netology_auth import ensure_netology_login
from dotenv import load_dotenv
from backend.summary.generate_summary import generate_summary, classify_lesson_via_llm
    ...
    if video_url and (not text or len(text) < 1000):
        ...
        try:
            audio_path = extract_audio_from_mp4(
                video_url, output_dir=f"backend/netology_cookies/audio/{safe_filename(subject_name)}"
            )
